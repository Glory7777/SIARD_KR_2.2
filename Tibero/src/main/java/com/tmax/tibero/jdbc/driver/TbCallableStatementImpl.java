...
public class TbCallableStatementImpl extends TbPreparedStatementImpl implements CallableStatement {
  ...
  public void addPivotData(int paramInt, byte[] paramArrayOfbyte) {
    if (this.pivotDataArr == null) {
      this.pivotDataArr = new ArrayList<Vector<byte[]>>(this.bindParamCnt);
      for (byte b = 0; b < this.bindParamCnt; b++)
  ...
  private Array getArrayInternal(int paramInt) throws SQLException {
    ...
    return this.typeConverter.toArray(arrayOfByte, 40, bindItem.getLength(), DataType.getDataType(bindItem.getSQLType()), !getRealRsetType().isScrollable(), tbArrayDescriptor, this.conn.getTypeMap());
  }
  ...
  private Object getObjectInternal(int paramInt) throws SQLException {
    ...
    return setLastColumnWasNull(bindItem) ? null : this.typeConverter.toObject(this.outParamBytes[paramInt - 1], 0, bindItem.getLength(), this.outParamTypes[paramInt - 1], bindItem.getSQLType(), true);
  }
