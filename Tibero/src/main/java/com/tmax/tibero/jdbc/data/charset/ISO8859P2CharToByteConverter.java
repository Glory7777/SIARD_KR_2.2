  ...
  public int convCharArr(char[] paramArrayOfchar, int paramInt1, int paramInt2, byte[] paramArrayOfbyte, int paramInt3, int paramInt4) throws SQLException {
    ...
    while (i < paramInt2 && j < paramInt4) {
      ...
      if (c < ' ') {
        ...
        byte b;
        if (c >= ' ' && c < 'ƀ') {
          b = (byte) UNICODE_TO_ISO8859P2_IDX_PAGE00[c - 160];
        } else if (c >= 'ˀ' && c < 'ˠ') {
          b = (byte) UNICODE_TO_ISO8859P2_IDX_PAGE02[c - 704];
        } else {
          b = 63;
        } 
        if (b == 0)
...
/* Location:              C:\TmaxData\tibero6\client\lib\jar\tibero6-jdbc.jar!\com\tmax\tibero\jdbc\data\charset\ISO8859P2CharToByteConverter.class
