...
public class TbArrayDataResultSet implements TbResultSet {
  ...
  public <T> T getObject(int i, Class<T> clazz) throws SQLException {
    Debug.logMethod("TbArrayDataResultSet.getObject", new Object[]{this, Integer.toString(i), clazz});
  ...
  private OffsetTime getOffsetTime(int i) throws SQLException {
    ...
    if (this.setLastColumnIsNull(i)) {
      return null;
    } else {
      byte[] value = this.getData(i);
      if (value.length == 17) {
        TimeZone timezone = this.typeConverter.toTimeZoneFromBytes(value, 0, value.length);
