...
public class TableImpl extends SearchImpl implements Table {
  ...
  static DocumentBuilder getDocumentBuilder() throws IOException {
    try {
      if (_db == null) {
        ...
        DocumentBuilderFactory dbf = DocumentBuilderFactory.newInstance(DocumentBuilderFactoryImpl.class
            .getName(), null);
  ...
  private static Transformer getTransformer() throws IOException {
    try {
      if (_trans == null) {
        ...
        TransformerFactory tf = TransformerFactory.newInstance(TransformerFactoryImpl.class
            .getName(), null);
  ...
  private SortedTable _stable = null; public SortedTable getSortedTable() {
    return this._stable;
  }
  ...
  private MetaTable _mt = null; private boolean _bCreating;
  ...
  private void addElement(Element elParent, String sTag, MetaValue mv, boolean bNullable) throws IOException {
    ...
    if (iPreType != 0 && mv.getMetaFields() == 0) {
      ...
      switch (iPreType) {
        case 1:
          sXmlType = bShort ? "xs:string" : "clobType"; break;
        case 12: sXmlType = bShort ? "xs:string" : "clobType"; break;
        case 70: sXmlType = "blobType"; break;
        case 2005: sXmlType = "clobType"; break;
        case -15: sXmlType = bShort ? "xs:string" : "clobType"; break;
        case -9: sXmlType = bShort ? "xs:string" : "clobType"; break;
        case 2011: sXmlType = "clobType"; break;
        case -2: sXmlType = bShort ? "xs:hexBinary" : "blobType"; break;
        case -3: sXmlType = bShort ? "xs:hexBinary" : "blobType"; break;
        case 2004: sXmlType = "blobType"; break;
        case 2: sXmlType = "xs:decimal"; break;
        case 3: sXmlType = "xs:decimal"; break;
        case 5: sXmlType = "xs:integer"; break;
        case 4: sXmlType = "xs:integer"; break;
        case -5: sXmlType = "xs:integer"; break;
        case 6: sXmlType = "xs:double"; break;
        case 7: sXmlType = "xs:float"; break;
        case 8: sXmlType = "xs:double"; break;
        case 16: sXmlType = "xs:boolean"; break;
        case 91: sXmlType = "dateType"; break;
        case 92: sXmlType = "timeType"; break;
        case 93: sXmlType = "dateTimeType"; break;
        case 2009: sXmlType = "clobType"; break;
        case 1111: sXmlType = "xs:duration"; break;
      } 
      ...
      for (int iField = 0; iField < mv.getMetaFields(); iField++) {
        ...
        addElement(elSequence, sTagField, (MetaValue)metaField, true);
      } 
    } 
  }
  ...
  public void exportTableSchema(OutputStream osXsd) throws IOException {
    ...
    if (getArchiveImpl().existsFileEntry(sEntryName)) {
      ...
      InputStream is = getArchiveImpl().openFileEntry(sEntryName); int iRead;
  ...
  private TableImpl(Schema schemaParent, String sName) throws IOException { this._bCreating = false; this._schemaParent = schemaParent; MetaSchemaImpl msi = (MetaSchemaImpl)getParentSchema().getMetaSchema(); TablesType tts = msi.getSchemaType().getTables(); if (tts == null) { tts = _OF.createTablesType(); msi.getSchemaType().setTables(tts); }  TableType tt = null; for (int iTable = 0; tt == null && iTable < tts.getTable().size(); iTable++) { TableType ttTry = tts.getTable().get(iTable); if (sName.equals(ttTry.getName()))
        tt = ttTry;  }  SchemaImpl si = (SchemaImpl)getParentSchema(); if (tt == null) { String sFolder = "table" + String.valueOf(tts.getTable().size()); ArchiveImpl ai = getArchiveImpl(); ai.createFolderEntry(si.getSchemaFolder() + sFolder + "/"); tt = MetaTableImpl.createTableType(sName, sFolder); tts.getTable().add(tt); }  this._mt = MetaTableImpl.newInstance(this, tt); si.registerTable(sName, this); } public boolean isCreating() { return this._bCreating; } public void setCreating(boolean bCreating) {
  ...
  private void writeUdtValue(Writer wr, Value value, File folderLobs) throws IOException {
    ...
    for (int iAttribute = 0; iAttribute < value.getAttributes(); iAttribute++) {
      ...
      wr.write("</dt>\r\n");
      wr.write("  <dd>");
      writeValue(wr, (Value)value.getAttribute(iAttribute), folderLobs);
      wr.write("</dd>\r\n");
    } 
    wr.write("</dl>\r\n");
  }
  ...
  private void writeArrayValue(Writer wr, Value value, File folderLobs) throws IOException {
    ...
    for (int iElement = 0; iElement < value.getElements(); iElement++) {
      ...
      wr.write("  <li>");
      writeValue(wr, (Value)value.getElement(iElement), folderLobs);
      wr.write("</li>\r\n");
    } 
    wr.write("</ol>\r\n");
  }
  ...
  public void exportAsHtml(OutputStream os, File folderLobs) throws IOException {
    ...
    oswr.write("      </tr>\r\n");
    RecordDispenser rd = openRecords(); long lRow;
    for (lRow = 0L; lRow < getMetaTable().getRows(); lRow++) {
      ...
      for (int i = 0; i < record.getCells(); i++) {
        ...
        writeValue(oswr, (Value)cell, folderLobs);
        oswr.write("</td>\r\n");
      } 
      oswr.write("      </tr>\r\n");
    } 
...
/* Location:              C:\Users\lenovo\IdeaProjects\SIARD_KR_2.2\lib\siardapi.jar!\ch\admin\bar\siard2\api\primary\TableImpl.class
