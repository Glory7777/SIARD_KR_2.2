  ...
  implements RecordDispenser
{
  ...
  private CountingInputStream _isXml = null; InputStream getXmlInputStream() {
    return this._isXml;
  } private XMLStreamReader _xsr = null; XMLStreamReader getXmlStreamReader() {
    return this._xsr;
  ...
  private Element getRowElement(XMLStreamReader xsr, Document doc) throws XMLStreamException {
    ...
    if (xsr.isStartElement() && sTag.equals("row")) {
      ...
      xsr.next(); for (; listElementsStack.size() > 0; xsr.next()) {
        Element elNew; int i; Element elParent; Text text;
  ...
  public Record get() throws IOException {
    ...
    try {
      ...
      throw new IOException("Record " + String.valueOf(this._lRecord) + " cannot be read!", xse);
    }  return record;
  }
  ...
  public void skip(long lSkip) throws IOException {
    ...
    try { if (this._lRecord + lSkip > this._table.getMetaTable().getRows())
    ...
    catch (XMLStreamException xse) { throw new IOException(String.valueOf(lSkip) + " records starting with " + String.valueOf(this._lRecord) + " could not be skipped (" + EU.getExceptionMessage(xse) + ")!"); }
...
/* Location:              C:\Users\lenovo\IdeaProjects\SIARD_KR_2.2\lib\siardapi.jar!\ch\admin\bar\siard2\api\primary\RecordDispenserImpl.class
