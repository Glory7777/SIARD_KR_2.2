  ...
  extends JAXBContext
{
    ...
    implements ValidationEventHandler
  {
    ...
    public boolean handleEvent(ValidationEvent ve) {
      ...
      ValidatingJAXBContext.m_il.enter(ve);
      if (ve.getSeverity() == 1 || ve
        .getSeverity() == 2) {
        
        ValidationEventLocator vel = ve.getLocator();
        ValidatingJAXBContext.m_il.severe("XML validation for " + vel.getURL() + " failed at line " +
                vel.getLineNumber() + " and column " +
                vel.getColumnNumber() + " with message " + ve
            .getMessage());
      } else {
  ...
  public Unmarshaller createUnmarshaller() throws JAXBException {
    ...
    if (this.m_urlSchema != null) {
      ...
      { throw new RuntimeException("Unable to create Schema " + this.m_urlSchema + "!", se); }
      catch (IOException ie) { throw new RuntimeException("Unable to read Schema " + this.m_urlSchema + "!", ie); }
...
/* Location:              C:\Users\lenovo\IdeaProjects\siardapi.jar!\ch\entera\\utils\jaxb\ValidatingJAXBContext.class
