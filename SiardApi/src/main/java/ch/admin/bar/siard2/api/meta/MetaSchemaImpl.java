...
public class MetaSchemaImpl extends MetaSearchImpl implements MetaSchema {
  ...
  private final Map<String, MetaType> _mapMetaTypes = new HashMap<>();
  private final Map<String, MetaView> _mapMetaViews = new HashMap<>();
  private final Map<String, MetaRoutine> _mapMetaRoutines = new HashMap<>();
  ...
  public void setTemplate(SchemaType stTemplate) throws IOException {
    ...
    if (rts != null)
    {
      for (int iRoutine = 0; iRoutine < rts.getRoutine().size(); iRoutine++) {
        ...
        if (mr != null && mr instanceof MetaRoutineImpl mri) {
  ...
  public MetaTable getMetaTable(int iTable) {
    String sName = this._st.getTables().getTable().get(iTable).getName();
...
/* Location:              C:\Users\lenovo\IdeaProjects\siardapi.jar!\ch\admin\bar\siard2\api\meta\MetaSchemaImpl.class
