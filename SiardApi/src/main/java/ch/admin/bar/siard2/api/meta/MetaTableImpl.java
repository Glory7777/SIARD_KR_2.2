...
public class MetaTableImpl extends MetaSearchImpl implements MetaTable {
    ...
    private final Map<String, MetaForeignKey> _mapMetaForeignKeys = new HashMap<>();
    private final Map<String, MetaUniqueKey> _mapMetaCandidateKeys = new HashMap<>();
    private final Map<String, MetaCheckConstraint> _mapMetaCheckConstraints = new HashMap<>();
    private final Map<String, MetaTrigger> _mapMetaTriggers = new HashMap<>();
    ...
    protected MetaSearch[] getSubMetaSearches() throws IOException {
        ...
        for (int iForeignKey = 0; iForeignKey < getMetaForeignKeys(); iForeignKey++)
            ams[getMetaColumns() + iPrimaryKeys + getMetaCandidateKeys() + iForeignKey] = getMetaForeignKey(iForeignKey);
        for (int iCheckConstraint = 0; iCheckConstraint < getMetaCheckConstraints(); iCheckConstraint++)
            ams[getMetaColumns() + iPrimaryKeys + getMetaCandidateKeys() + getMetaForeignKeys() + iCheckConstraint] = getMetaCheckConstraint(iCheckConstraint);
        for (int iTrigger = 0; iTrigger < getMetaTriggers(); iTrigger++)
            ams[getMetaColumns() + iPrimaryKeys + getMetaCandidateKeys() + getMetaForeignKeys() + getMetaCheckConstraints() + iTrigger] = getMetaTrigger(iTrigger);
        return ams;
    }
...
/* Location:              C:\Users\lenovo\IdeaProjects\siardapi.jar!\ch\admin\bar\siard2\api\meta\MetaTableImpl.class
