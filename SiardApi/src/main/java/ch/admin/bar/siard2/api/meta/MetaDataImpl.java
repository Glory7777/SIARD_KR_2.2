...
public class MetaDataImpl extends MetaSearchImpl implements MetaData {
  private static final DU _du = DU.getInstance("en", "yyyy-MM-dd"); public static final String _sURI_SCHEME_FILE = "file";
  private static final ObjectFactory _of = new ObjectFactory();
  private final Map<String, MetaUser> _mapMetaUsers = new HashMap<>();
  private final Map<String, MetaRole> _mapMetaRoles = new HashMap<>();
  private final List<MetaPrivilege> _listMetaPrivileges = new ArrayList<>();
  ...
  public MetaSchema getMetaSchema(int iSchema) {
    String sName = XU.fromXml(this._sa.getSchemas().getSchema().get(iSchema).getName());
...
/* Location:              C:\Users\lenovo\IdeaProjects\siardapi.jar!\ch\admin\bar\siard2\api\meta\MetaDataImpl.class
