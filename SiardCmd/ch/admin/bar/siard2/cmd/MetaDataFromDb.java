...
public class MetaDataFromDb extends MetaDataBase {
  ...
  public static MetaDataFromDb newInstance(DatabaseMetaData dmd, MetaData md) throws SQLException { return new MetaDataFromDb(dmd, md); } public MetaColumn getMaxLobColumn() { return this._mcMaxLob; } private boolean checkMetaTable(MetaTable mt) {
  ...
  private String getReferentialAction(int iReferentialAction) {
    ...
    switch (iReferentialAction) {
      ...
      case 1:
        ...
        break;
      case 3:
        rat = ReferentialActionType.NO_ACTION; break;
    } 
    return rat.value();
  }
    ...
    while (rs.next()) {
      ...
      if (cat == CategoryType.DISTINCT) {
        ...
        if (iDataType != 2001 && iDataType != 2003 && iDataType != 2002)
          mt.setBasePreType(iDataType, iAttrSize, iDecimalDigits);  continue;
      } 
      MetaAttribute ma = mt.createMetaAttribute(sAttributeName);
      if (iDataType != 2001 && iDataType != 2003 && iDataType != 2002)
      { if (iDataType == 1111) { ma.setType(sAttrTypeName); }
      ...
      else { ma.setTypeName(mtAttr.getName());
        ma.setTypeSchema(mtAttr.getParentMetaSchema().getName()); }
      
      int iNullable = rs.getInt("NULLABLE");
      if (iNullable == 0) { ma.setNullable(false); }
      else if (iNullable == 1) { ma.setNullable(true); }
    ...
    while (rs.next()) {
      ...
      if (sSpecificName.equals(mr.getSpecificName())) {
        if (iColumnType == 5 || iColumnType == 3) {
          ...
          if (iDataType != 2001 && iDataType != 2003 && iDataType != 2002)
            mr.setReturnPreType(iDataType, lPrecision, iScale);  continue;
        } 
        ...
        else { mp.setTypeName(mt.getName());
  ...
  private void getColumnData(ResultSet rs, MetaColumn mc) throws IOException, SQLException {
    ...
    if (iDataType != 2001 && iDataType != 2003 && iDataType != 2002)
    { if (iDataType != 1111) { mc.setPreType(iDataType, lColumnSize, iDecimalDigits); }
    ...
    else { mc.setTypeName(mty.getName());
      mc.setTypeSchema(mty.getParentMetaSchema().getName()); }
    
    int iNullable = rs.getInt("NULLABLE");
    if (mc.getParentMetaTable() != null)
      if (iNullable == 0) { mc.setNullable(false); }
      else if (iNullable == 1) { mc.setNullable(true); }
  ...
  private void getUniqueKeys(MetaTable mt) throws IOException, SQLException {
    ...
    while (rs.next()) {
      ...
      LOG.debug("Metadata for unique key '{}' (table '{}.{}') loaded", new Object[] { sUniqueKeyName, sTableSchema, sTableName });
    } 
  ...
  private void getForeignKeys(MetaTable mt) throws IOException, SQLException {
    ...
    while (rs.next()) {
      ...
      LOG.debug("Metadata for foreign key '{}' (table '{}.{}') loaded", new Object[] { sForeignKeyName, sPkTableSchema, sPkTableName });
    } 
  ...
  private void getPrimaryKey(MetaTable mt) throws IOException, SQLException {
    ...
    while (rs.next()) {
      ...
      if (s != null) {
        ...
        LOG.info("No name for primary key of column '{}.{}.{}' available. Used '{}' instead.", new Object[] { sTableSchema, sTableName, sColumnName, sPkName });
      } 
      ...
      LOG.debug("Metadata for primary key '{}' (column '{}.{}.{}') loaded", new Object[] { sPkName, sTableSchema, sTableName, sColumnName });
    } 
  ...
  private void getColumns(MetaTable mt) throws IOException, SQLException {
    ...
    while (rs.next()) {
      ...
      LOG.debug("Metadata for column '{}.{}.{}' loaded", new Object[] { sTableSchema, sTableName, sColumnName });
    } 
  ...
  private void getTables() throws IOException, SQLException {
    ...
    while (rs.next() && !cancelRequested()) {
      ...
      String sTableName = rs.getString("TABLE_NAME");
      String sTableType = rs.getString("TABLE_TYPE");
      if (!Arrays.<String>asList(asTypes).contains(sTableType)) throw new IOException("Invalid table type found!"); 
      String sRemarks = rs.getString("REMARKS");
  ...
  public void download(boolean bViewsAsTables, boolean bMaxLobNeeded, Progress progress) throws IOException, SQLException {
    LOG.info("Start meta data download to archive {} (view-as-tables: {}, max-lob-needed: {})", new Object[] { this._md
