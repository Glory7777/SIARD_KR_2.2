...
public class TempRecordRetainerImpl implements RecordRetainer {

    private static final int iBUFFER_SIZE = 8192;
    private Table _table = null;
    private TempRecordRetainerImpl.CountingOutputStream _osXml = null;
    ...
    static XMLStreamWriter writeHeader(OutputStream osXml, Table table) throws IOException {
        ...
        try {
            ...
            xsw.writeNamespace((String)null, "http://www.bar.admin.ch/xmlns/siard/2/table.xsd");
    ...
    public TempRecordRetainerImpl(Table table) throws IOException {
        CustomTableImpl ti = (CustomTableImpl) table;
        ti.setCreating(true);
        this._table = table;
        if (this.getArchiveImpl().canModifyPrimaryData()) {
            ...
            this._lRecord = 0L;
            this._osXml = new TempRecordRetainerImpl.CountingOutputStream(this.getArchiveImpl().createFileEntry(ti.getTableXml()));
...
//        putRecordType(((RecordImpl)record).getRecordType(), xsw);
            ...
            if (afile != null) {
                for(int i = 0; i < afile.length; ++i) {
                    ...
                    if (file.isDirectory()) {
                        ...
                        for(int iRead = ((InputStream)isLob).read(buffer); iRead != -1; iRead = ((InputStream)isLob).read(buffer)) {
                            osLob.write(buffer, 0, iRead);
                        }

                        ((InputStream)isLob).close();
                        osLob.close();
                        file.delete();
                    }
                }
    ...
    public void close() throws IOException {
        try {
            ...
            CustomTableImpl ti = (CustomTableImpl) this._table;
