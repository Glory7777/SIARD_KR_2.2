...
public class ReaderBinder extends Binder {
  ...
  public void bindDFR(TbConnection paramTbConnection, ParamContainer paramParamContainer, TbStreamDataWriter paramTbStreamDataWriter, int paramInt1, int paramInt2, long paramLong) throws SQLException {
    ...
    char[] arrayOfChar = new char[paramTbConnection.getMaxDFRCharCount()];
    ...
    try {
      while (paramLong > 0L) {
        ...
        if (k == -1)
          break; 
        int m = k * dataTypeConverter.getMaxBytesPerChar();
        paramTbStreamDataWriter.makeBufferAvailable(m + 4);
        byte[] arrayOfByte = paramTbStreamDataWriter.getStreamBuf().getRawBytes();
        int j = dataTypeConverter.charsToBytes(arrayOfChar, 0, k, arrayOfByte, i + 4, m);
...
/* Location:              C:\Users\lenovo\Desktop\siard\jdbc\tibero\tibero6\linux\tibero6-jdbc-18.jar!\com\tmax\tibero\jdbc\data\binder\ReaderBinder.class
