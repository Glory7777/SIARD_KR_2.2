...
public class TbArrayDataResultSet implements TbResultSet {
  ...
  private OffsetTime getOffsetTime(int paramInt) throws SQLException {
    ...
    if (setLastColumnIsNull(paramInt))
      return null; 
    byte[] arrayOfByte = getData(paramInt);
    if (arrayOfByte.length == 17) {
      TimeZone timeZone = this.typeConverter.toTimeZoneFromBytes(arrayOfByte, 0, arrayOfByte.length);
...
/* Location:              C:\Users\lenovo\Desktop\siard\jdbc\tibero\tibero6\linux\tibero6-jdbc-18.jar!\com\tmax\tibero\jdbc\TbArrayDataResultSet.class
