...
public class TbCachedRowSet extends TbRowSet implements RowSetInternal, Serializable, CachedRowSet, Joinable {
    ...
    public CachedRowSet createCopy() throws SQLException {
        ...
        tbCachedRowSet.rows = new Vector<>(i);
        for (byte b = 0; b < i; b++)
    ...
    public byte getByte(int paramInt) throws SQLException {
        ...
        if (object instanceof TbBlob) {
            ...
            return tbBlob.getBytes(1L, 1)[0];
        }
        if (object instanceof TbClobBase) {
            TbClobBase tbClobBase = (TbClobBase) object;
            return tbClobBase.getSubString(1L, 1).getBytes()[0];
        }
        throw TbError.newSQLException(-90836, object.toString());
    }
    ...
    public byte[] getBytes(int paramInt) throws SQLException {
        ...
        if (object instanceof TbClobBase) {
            TbClobBase tbClobBase = (TbClobBase) object;
            return tbClobBase.getSubString(1L, (int) tbClobBase.length()).getBytes();
        }
        throw TbError.newSQLException(-90836, object.toString());
    }
    ...
    private String getStringInternal(int paramInt) throws SQLException {
        ...
        if (object instanceof TbClobBase) {
            TbClobBase tbClobBase = (TbClobBase) object;
            return tbClobBase.getSubString(1L, (int) tbClobBase.length());
        }
        if (object instanceof TbBlob) {
            TbBlob tbBlob = (TbBlob) object;
            return new String(tbBlob.getBytes(1L, (int) tbBlob.length()));
        }
    ...
    private Time getTimeInternal(int paramInt, Calendar paramCalendar) throws SQLException {
        ...
        if (object instanceof Timestamp) {
            if (paramCalendar == null)
                paramCalendar = Calendar.getInstance();
            paramCalendar.setTime((Timestamp) object);
    ...
    public <T> T getObject(int paramInt, Class<T> paramClass) throws SQLException {
        ...
        switch (str) {
            ...
            case "java.time.OffsetTime":
                return (T) OffsetTime.of(getTime(paramInt).toLocalTime(), ZoneOffset.UTC);
            case "java.time.OffsetDateTime":
                return (T) OffsetDateTime.of(getTimestamp(paramInt).toLocalDateTime(), ZoneOffset.UTC);
            case "java.net.URL":
...
/* Location:              C:\Users\lenovo\Desktop\siard\jdbc\tibero\tibero6\linux\tibero6-jdbc-18.jar!\com\tmax\tibero\jdbc\rowset\TbCachedRowSet.class
