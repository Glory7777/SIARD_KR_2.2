...
public class SJISEncoder implements CharsetEncoder {
  private SJISCharToByteConverter conv = null;
  
  private int maxBytesPerChar = 0;
  
  public SJISEncoder() {
    this.conv = new SJISCharToByteConverter();
    this.conv.subBytes = new byte[] { -127, 72 };
  ...
  public boolean isEndingByte(byte[] paramArrayOfbyte, int paramInt) {
    return isSingleByte(paramArrayOfbyte[paramInt]) ? true : (!isLeadingByte(paramArrayOfbyte, paramInt));
  }
...
/* Location:              C:\Users\lenovo\Desktop\siard\jdbc\tibero\tibero6\linux\tibero6-jdbc-18.jar!\com\tmax\tibero\jdbc\data\charset\SJISEncoder.class
