...
public class TbStatement extends com.tmax.tibero.jdbc.TbStatement {
    ...
    protected Vector<byte[]> pivotData = (Vector) new Vector<>();
    ...
    private synchronized int executeInternal(String paramString, Object paramObject) throws SQLException {
        ...
        if (this.originalSql.length() > 65535 / i && this.sqlType != 128) {
    ...
    public synchronized long executeLargeUpdate(String paramString) throws SQLException {
        checkConnectionOpen();
        if (this.batchStmts != null) {
            checkBatchStmtRemained();
            initBatchStmts();
        ...
        try {
            this.isExecuting = true;
            return executeLargeInternal(paramString);
        } finally {
            this.isExecuting = false;
            if (this.queryTimeout != 0)
                this.conn.getTimeout().cancelTimeout();
    ...
    public long executeLargeUpdate(String paramString, int paramInt) throws SQLException {
        if (paramInt == 2)
            return executeLargeUpdate(paramString);
        if (paramInt != 1)
            TbError.newSQLException(-590733);
        checkConnectionOpen();
        if (this.batchStmts != null) {
    ...
    public long executeLargeUpdate(String paramString, int[] paramArrayOfint) throws SQLException {
        ...
        try {
            this.isExecuting = true;
            return executeLargeInternal(paramString, paramArrayOfint);
        } finally {
            this.isExecuting = false;
            if (this.queryTimeout != 0)
                this.conn.getTimeout().cancelTimeout();
    ...
    public long executeLargeUpdate(String paramString, String[] paramArrayOfString) throws SQLException {
        ...
        try {
            this.isExecuting = true;
            return executeLargeInternal(paramString, paramArrayOfString);
        } finally {
            this.isExecuting = false;
            if (this.queryTimeout != 0)
                this.conn.getTimeout().cancelTimeout();
    ...
    private synchronized long executeLargeInternal(String paramString) throws SQLException {
        return executeLargeInternal(paramString, null);
    }

    private synchronized long executeLargeInternal(String paramString, Object paramObject) throws SQLException {
        initSql(paramString);
        if (!isInsertStmt(paramString))
            this.returnAutoGeneratedKeys = false;
        int i = this.conn.typeConverter.getMaxBytesPerChar();
        if (this.originalSql.length() > 65535 / i) {
            ...
            if (j > 0) {
                ...
                arrayList.clear();
                return tbPreparedStatement.impl().executeLargeInternal(str);
            }
        }
        if (TbSQLTypeScanner.isQueryStmt(this.sqlType)) {
            if (this.userRsetType.getType() == 1005 || this.userRsetType.getConcurrency() == 1008) {
                ...
                try {
                    this.realRsetType = this.userRsetType;
                    synchronized (this.conn) {
                        return this.conn.getTbComm().executeLargeDirect(this, this.sqlWithRowId);
                    }
                } catch (SQLException sQLException) {
                    this.realRsetType = RsetType.getDownGradedRsetType(this.userRsetType.getRank());
                    synchronized (this.conn) {
                        return this.conn.getTbComm().executeLargeDirect(this, this.originalSql);
                    }
                }
            }
            try {
                ...
                if (this.conn.isClosed() || !this.conn.info.isFailoverCursorEnabled() || sQLException.getErrorCode() != -90700)
                    throw sQLException;
                synchronized (this.conn) {
                    long l = this.conn.getTbComm().executeLargeDirect(this, this.originalSql);
                    addWarning(TbError.newSQLWarning(-90700, sQLException));
                    return l;
                }
            }
        ...
        if (TbSQLTypeScanner.isDMLStmt(this.sqlType) || (!TbSQLTypeScanner.isQueryStmt(this.sqlType) && !TbSQLTypeScanner.isPSMStmt(this.sqlType))) {
            ...
            synchronized (this.conn) {
                this.rowsUpdated = this.conn.getTbComm().executeLargeDirect(this, str);
            }
...
/* Location:              C:\Users\lenovo\Desktop\siard\jdbc\tibero\tibero6\linux\tibero6-jdbc-18.jar!\com\tmax\tibero\jdbc\driver\TbStatement.class
