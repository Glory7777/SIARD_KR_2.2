...
public enum ErrorCode {
    ...
    ACCESS_DENIED(MYSQL, 1044, ExceptionLevel.CRITICAL, "스키마에 대한 접근 권한이 부족합니다. 유저 SELECT 권한을 확인해주세요", null),
    INSUFFICIENT_PRIVILEGE_CREATE_SESSION(ORACLE, 1045, ExceptionLevel.CRITICAL, "커넥션 세션에 대한 권한이 없습니다. 유저에 CREATE SESSION 권한을 부여해주세요", null),
    TABLE_OR_VIEW_NOT_FOUND(ORACLE, 942, ExceptionLevel.INFO, "테이블 사이즈를 읽어오는 데 실패했습니다. 유저에게 테이블 dba_segments에 대한 SELECT 권한이 필요합니다.", "getTableSize"),
    CANNOT_COMMUNICATE_BROKER(CUBRID, -21003, ExceptionLevel.CRITICAL, "DB 서버(브로커)와의 통신에 문제가 발생했습니다. SIARD 프로그램을 종료 후 다시 시도해주세요.", null),
    INSUFFICIENT_AUTH(CUBRID, -494, ExceptionLevel.CRITICAL, "유저 권한이 부족합니다. 현재 유저에 _db_data_type 테이블에 대한 SELECT 권한이 필요합니다", null),
