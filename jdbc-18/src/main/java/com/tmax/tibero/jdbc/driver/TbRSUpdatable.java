...
public class TbRSUpdatable extends TbResultSet {
    ...
    public <T> T getObject(int paramInt, Class<T> paramClass) throws SQLException {
        ...
        switch (str) {
            ...
            case "java.time.LocalTime":
                return (T) getTime(paramInt).toLocalTime();
    ...
    private OffsetTime getOffsetTime(int paramInt) throws SQLException {
        ...
        if (setLastColumnWasNull(paramInt))
            return null;
        if (hasBufferChanged(paramInt)) {
            if (this.columnLength[getRevisedColumnIndex(paramInt)] == 17) {
                TimeZone timeZone = this.rset.typeConverter.toTimeZoneFromBytes((byte[]) this.columnBuffer[getRevisedColumnIndex(paramInt)], 0, this.columnLength[getRevisedColumnIndex(paramInt)]);
                return OffsetTime.of(getTimestamp(paramInt).toLocalDateTime().toLocalTime(), ZoneOffset.ofTotalSeconds(timeZone.getRawOffset() / 1000));
            }
            return OffsetTime.of(getTime(paramInt).toLocalTime(), ZoneOffset.UTC);
        }
        return this.rset.<OffsetTime>getObject(paramInt, OffsetTime.class);
    }
    ...
    private OffsetDateTime getOffsetDateTime(int paramInt) throws SQLException {
        ...
        if (hasBufferChanged(paramInt)) {
            if (this.columnLength[getRevisedColumnIndex(paramInt)] == 17) {
                TimeZone timeZone = this.rset.typeConverter.toTimeZoneFromBytes((byte[]) this.columnBuffer[getRevisedColumnIndex(paramInt)], 0, this.columnLength[getRevisedColumnIndex(paramInt)]);
                return OffsetDateTime.of(getTimestamp(paramInt).toLocalDateTime(), ZoneOffset.ofTotalSeconds(timeZone.getRawOffset() / 1000));
            }
            return OffsetDateTime.of(getTimestamp(paramInt).toLocalDateTime(), ZoneOffset.UTC);
        }
        return this.rset.<OffsetDateTime>getObject(paramInt, OffsetDateTime.class);
    }
