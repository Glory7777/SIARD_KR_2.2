...
public class StreamBinder extends Binder {
    ...
    public void bindDFR(TbConnection paramTbConnection, ParamContainer paramParamContainer, TbStreamDataWriter paramTbStreamDataWriter, int paramInt1, int paramInt2, long paramLong) throws SQLException, IOException {
        boolean bool1;
        boolean bool2;
        ...
        while (paramLong > 0L) {
            ...
            if (charFragmentsLen > 0) {
                j -= charFragmentsLen;
                System.arraycopy(arrayOfByte, 0, arrayOfByte1, k + 4, charFragmentsLen);
            }
            int n = inputStream.read(arrayOfByte1, k + 4 + charFragmentsLen, j);
            if (n == -1) {
                if (charFragmentsLen > 0) {
                    n = charFragmentsLen;
                    charFragmentsLen = 0;
                ...
                if (charFragmentsLen > 0) {
                    n -= charFragmentsLen;
                    System.arraycopy(arrayOfByte1, i2, arrayOfByte, 0, charFragmentsLen);
                }
            }
