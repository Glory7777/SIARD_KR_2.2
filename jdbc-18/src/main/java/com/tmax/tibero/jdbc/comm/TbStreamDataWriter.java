...
public class TbStreamDataWriter {
  ...
  public int writeLenAndSqlWithNPrefixed(String paramString) throws SQLException {
    int i = this.buf.getCurDataSize();
    int j = 0;
    ...
    for (byte b2 = 0; b2 < k; b2++) {
      if (paramString.charAt(b2) == '\'')
        if (b == -1) {
          b = b2;
        } else if (b2 + 1 < k && paramString.charAt(b2 + 1) == '\'') {
          b2++;
        } else {
          if (paramString.charAt(b - 1) == 'N' || paramString.charAt(b - 1) == 'n') {
            j += writeBytes(this.converter.getDBEncodedBytes(paramString.substring(b1, b + 1)));
            j += writeBytes(uTF8Encoder.stringToBytes(paramString.substring(b + 1, b2)));
            b1 = b2;
          } 
          b = -1;
        }  
    } 
    j += writeBytes(this.converter.getDBEncodedBytes(paramString.substring(b1, k)));
    reWriteInt(i, j, 4);
    return j + writePadding(j) + 4;
  }
...
/* Location:              C:\Users\lenovo\Desktop\siard\jdbc\tibero\tibero6\linux\tibero6-jdbc-18.jar!\com\tmax\tibero\jdbc\comm\TbStreamDataWriter.class
