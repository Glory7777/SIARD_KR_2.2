...
public class ArrayInBinder extends Binder {
    ...
    public void bind(TbConnection paramTbConnection, ParamContainer paramParamContainer, TbStreamDataWriter paramTbStreamDataWriter, int paramInt1, int paramInt2, int paramInt3) throws SQLException {
        ...
        TbArray tbArray = (TbArray) paramParamContainer.getParamArray(paramInt1, paramInt2);
        if (tbArray == null) {
        ...
        byte b3 = 0;
        LinkedList<UdtNode> udtTree = new LinkedList<>();
        ArrayUdtNode root = new ArrayUdtNode(tbArray);
        udtTree.addFirst(root);
        UdtNode cur = root;
        ...
        while (!udtTree.isEmpty()) {
            ...
            if (arrayOfObject == null) {
                ...
                if (udtTree.isEmpty())
                    continue;
                cur = udtTree.getFirst();
                continue;
            }
            int i8 = cur.attrProcessed;
            cur.attrProcessed++;
            Object object = arrayOfObject[i8];
            int i9 = 0;
            if (cur instanceof ObjUdtNode) {
                i9 = ((TbStructDescriptor) cur.getDescriptor()).getAttributeTypes()[i8];
            } else if (cur instanceof ArrayUdtNode) {
                i9 = ((TbArrayDescriptor) cur.getDescriptor()).getElementType();
            } else {
                ...
                case 32:
                    if (object instanceof TbStruct || object instanceof SQLData) {
                        ...
                        if (object instanceof SQLData) {
                            ...
                            newCtx = new ObjUdtNode(tbStruct);
                        } else {
                            newCtx = new ObjUdtNode((TbStruct) object);
                        }
                        if (!((TbStructDescriptor) newCtx.getDescriptor()).isFinal()) {
                            int[] arrayOfInt1 = ObjUdtNode.writeNotFinalObject(paramTbStreamDataWriter, (TbStructDescriptor) newCtx.desc);
                            udtTree.addFirst(newCtx);
                            cur = newCtx;
                            cur.setOffset(arrayOfInt1);
                            continue;
                        }
                        int[] arrayOfInt = ObjUdtNode.writeNotFinalObject(paramTbStreamDataWriter, (TbStructDescriptor) newCtx.desc);
                        udtTree.addFirst(newCtx);
                        cur = newCtx;
            ...
            while (cur.attrProcessed >= cur.getNumOfFields()) {
                int i11 = 0;
                if (cur instanceof ObjUdtNode) {
                    i11 = ((TbStructDescriptor) cur.getDescriptor()).getAttributeTypes()[i8];
                } else if (cur instanceof ArrayUdtNode) {
                    i11 = ((TbArrayDescriptor) cur.getDescriptor()).getElementType();
                }
                if (udtTree.size() != 1)
