...
public class TbDatabaseMetaQuery {
  ...
  public static String QUERY_CATALOGS = " SELECT 'nothing' as catalogs  FROM SYS._VT_DUAL  WHERE 1 = 2";
  ...
  public static String QUERY_TABLETYPES = " select 'TABLE' as table_type  from SYS._VT_DUAL  union select 'VIEW' as table_type  from SYS._VT_DUAL  union select 'SYNONYM' as table_type  from SYS._VT_DUAL ";
  ...
  public static String QUERY_UDTMETA__BY_OID = "SELECT CLIENT_INTERNAL.GETUDTMETA(?) UDT_META FROM SYS._VT_DUAL";
  
  public static String QUERY_UDTMETA__BY_TYPENAME = "SELECT CLIENT_INTERNAL.GETUDTMETABYNAME(?, ?) UDT_META FROM SYS._VT_DUAL";
  ...
  private static String QUERY_BESTROWIDENTIFIER_1 = " SELECT 1 AS scope,  'ROWID' AS column_name,  1 AS data_type,  'ROWID' AS type_name,  0 AS column_size,  0 AS buffer_length,  0 AS decimal_digits,  2 AS pseudo_column  FROM SYS._VT_DUAL  WHERE ? = 1  UNION  SELECT 2 AS scope,  t.column_name, ";
  
  private static String QUERY_BESTROWIDENTIFIER_2 = " DECODE (NVL(t.data_precision, 0), 0, t.data_length, t.data_precision) AS column_size,  0 AS buffer_length, t.data_scale AS decimal_digits,  1 AS pseudo_column  FROM all_tab_columns t,  all_ind_columns i  WHERE ? = 1  AND t.table_name = ?  AND t.owner like ? escape '" + escapeChar + "'  AND t.nullable != ?  AND t.owner = i.table_owner  AND t.table_name = i.table_name  AND t.column_name = i.column_name ";
  ...
  public static String getBestRowIdentifierQuery(boolean paramBoolean) {
    ...
    stringBuffer.append(CLAUSE_COLUMN_TYPE_DECODE_1.replace('b', 't')).append(" AS data_type, ");
    if (paramBoolean)
  ...
  public static String getSetClientInfoQuery(HashMap<String, Boolean> paramHashMap) {
    StringBuffer stringBuffer = new StringBuffer();
    stringBuffer.append("declare ");
    stringBuffer.append("MODULE_NAME varchar(64); ");
    stringBuffer.append("ACTION_NAME varchar(64); ");
    stringBuffer.append("begin ");
    if (paramHashMap.get("ClientID") != null && ((Boolean)paramHashMap.get("ClientID")).booleanValue())
      stringBuffer.append("dbms_session.set_identifier(?); "); 
    if (paramHashMap.get("Module") != null && ((Boolean)paramHashMap.get("Module")).booleanValue()) {
      stringBuffer.append("dbms_application_info.read_module(MODULE_NAME, ACTION_NAME); ");
      stringBuffer.append("dbms_application_info.set_module(?, ACTION_NAME); ");
    } 
    if (paramHashMap.get("Action") != null && ((Boolean)paramHashMap.get("Action")).booleanValue())
      stringBuffer.append("dbms_application_info.set_action(?); "); 
    if (paramHashMap.get("ClientInfo") != null && ((Boolean)paramHashMap.get("ClientInfo")).booleanValue())
      stringBuffer.append("dbms_application_info.set_client_info(?); "); 
  ...
  public static String getPseudoColumnQuery() {
    ...
    stringBuffer.append("select null as TABLE_CAT, owner as TABLE_SCHEM, table_name as TABLE_NAME, 'ROWTSN' as COLUMN_NAME, ");
    stringBuffer.append("2 as DATA_TYPE, 10 as COLUMN_SIZE, 38 as DECIMAL_DIGITS, 10 as NUM_PREC_RADIX, ");
    stringBuffer.append("'NO_USAGE_RESTRICTONS' as COLUMN_USAGE, null as REMARKS, null as CHAR_OCTET_LENGTH,'NO' as IS_NULLABLE ");
    stringBuffer.append("from all_tables where owner like ? and ");
    stringBuffer.append("table_name like ? ESCAPE '").append(escapeChar).append("'");
    stringBuffer.append(" union ");
    stringBuffer.append("select null as TABLE_CAT, owner as TABLE_SCHEM, table_name as TABLE_NAME, 'ROWID' as COLUMN_NAME, ");
    stringBuffer.append("-8 as DATA_TYPE, 18 as COLUMN_SIZE, null as DECIMAL_DIGITS, 10 as NUM_PREC_RADIX, ");
    stringBuffer.append("'NO_USAGE_RESTRICTONS' as COLUMN_USAGE, null as REMARKS, null as CHAR_OCTET_LENGTH,'NO' as IS_NULLABLE ");
    stringBuffer.append("from all_tables where owner like ? and ");
    stringBuffer.append("table_name like ? ESCAPE '").append(escapeChar).append("'");
    stringBuffer.append(" union ");
    stringBuffer.append("select null as TABLE_CAT, owner as TABLE_SCHEM, table_name as TABLE_NAME, 'ROWNUM' as COLUMN_NAME, ");
    stringBuffer.append("2 as DATA_TYPE, 10 as COLUMN_SIZE, 38 as DECIMAL_DIGITS, 10 as NUM_PREC_RADIX, ");
    stringBuffer.append("'NO_USAGE_RESTRICTONS' as COLUMN_USAGE, null as REMARKS, null as CHAR_OCTET_LENGTH, 'NO' as IS_NULLABLE ");
    stringBuffer.append("from all_tables where owner like ? and ");
    stringBuffer.append("table_name like ? ESCAPE '").append(escapeChar).append("'");
    ...
    stringBuffer.append("DECODE (data_precision, 0, data_length, data_precision) AS column_size, ");
    stringBuffer.append("data_scale as DECIMAL_DIGITS, 10 as NUM_PREC_RADIX, 'NO_USAGE_RESTRICTONS' as COLUMN_USAGE, ");
    stringBuffer.append("null as REMARKS, null as CHAR_OCTET_LENGTH, decode(nullable, 'Y', 'YES', 'NO') as IS_NULLABLE from all_tab_cols ");
    stringBuffer.append("where hidden_column = 'YES' and owner like ? and ");
    stringBuffer.append("table_name like ? ESCAPE '").append(escapeChar).append("') ");
    stringBuffer.append("where column_name like ? order by column_name");
    return stringBuffer.toString();
  }
}


/* Location:              C:\Users\lenovo\Desktop\siard\jdbc\tibero\tibero6\linux\tibero6-jdbc-18.jar!\com\tmax\tibero\jdb\\util\TbDatabaseMetaQuery.class
